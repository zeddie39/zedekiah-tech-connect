...
const PricingPage: React.FC = () => {
  ...
  return (
    <PageTransition>
      ...
      />
      <div className="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-white flex flex-col transition-colors duration-300">
        <Navbar />
        <main className="flex-grow pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto w-full">
          ...
          <div className="text-center mb-12">
            ...
            <h2 className="text-2xl font-semibold text-gray-700 dark:text-slate-300 mb-6">SERVICE PRICE GUIDE (2026)</h2>
          ...
          <div className="space-y-12">
            {filteredData.map((category) => (
              <div key={category.id} className="bg-white dark:bg-slate-900/90 rounded-xl shadow-md overflow-hidden border border-gray-100 dark:border-slate-800">
                <div className="bg-accent/10 px-6 py-4 border-b border-accent/20 flex items-center gap-3">
                  <div className="p-2 bg-white dark:bg-slate-800 rounded-lg text-accent shadow-sm">
                ...
                {category.note && (
                  <div className="px-6 py-3 bg-gray-50 dark:bg-slate-950 text-sm text-gray-600 dark:text-slate-400 italic border-b border-gray-100 dark:border-slate-800">
                    * {category.note}
                  </div>
                )}
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-gray-50/50 dark:bg-slate-800/50">
                        <th className="px-6 py-3 text-sm font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wider border-b border-gray-200 dark:border-slate-700">Service</th>
                        <th className="px-6 py-3 text-sm font-semibold text-gray-600 dark:text-slate-400 uppercase tracking-wider border-b border-gray-200 dark:border-slate-700 text-right">Starting Price</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                      {category.items.map((item, idx) => (
                        <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-slate-200">
                            {item.service}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-right">
                            <span className="font-bold text-green-700 dark:text-emerald-400">{item.price}</span>
                            {item.note && <span className="text-gray-500 dark:text-slate-400 text-xs ml-2 italic">{item.note}</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
          ...
          <div className="mt-16 bg-white dark:bg-slate-900/90 rounded-xl shadow-md p-6 md:p-8 border border-gray-100 dark:border-slate-800">
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100 dark:border-slate-800">
              ...
              <h3 className="text-2xl font-bold text-gray-800 dark:text-white">Terms & Conditions</h3>
            </div>
            <ul className="space-y-3 text-gray-600 dark:text-slate-300 list-disc pl-5">
